...
def allocation_data(input_dir, acc):
    if acc:
        df_portfolio = pd.read_csv(f"{input_dir}/Portfolio_test_acc.csv") 
...
#     df_allocation["Our Portfolio"] = (df_allocation['Ticker'].map(allocation))/100
    ...
    df_metrics = opt_res*weight
    df_metrics = df_metrics.reset_index()
    df_metrics = df_metrics.rename(columns={'index': 'Type'})
    df_metrics = df_metrics.set_index("Type")
    df_metrics = df_metrics
#     df_metrics = df_metrics.rename(columns={"Min Volatility" : "Min Weighted Volatility", "Max Sharpe Ratio" : "Max Weighted Sharpe Ratio"})
#     df_metrics = df_metrics.T

    return fig_6, df_allocation, df_metrics

# @st.cache
def output_portfolio(input_dir, acc=False, num_sim=1000):
    
    df_portfolio = allocation_data(input_dir, acc)
    
    symbols, names, allocation, d = allocation_process(df_portfolio)
    fig_all_total, df_allocation, df_metrics = build_my_portfolio(d, names, start_date, end_date, num_sim=num_sim, weight=1)#, allocation=allocation)

    return fig_all_total, df_allocation, df_metrics