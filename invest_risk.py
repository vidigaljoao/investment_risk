...
header_image = Image.open("data/our_financial_journey.png")
...
st.subheader("Examples 5 Different Assets")
st.write(" ")
st.write("**0P0000SCQD.F** : Euro Government Bond Index Fund - low risk and return")
...
st.header("**Sharpe Ratio and Risk-Adjusted Returns measures of an investment**")
st.subheader("Historical data from the last 10 years - 5 Different Assets")
...
if new_data:    
    ...
    df_1 = pd.read_csv("data/risk-adjusted_data.csv")
    df_1['Assets'] = df_1['Unnamed: 0']
    df_1 = df_1.drop("Unnamed: 0", axis=1).set_index('Assets')
    
    efficient_image = Image.open("data/fig_efficient.png")
...
st.subheader("Investment Simulation in an All-World Index ETF (VWCE)")
...
with colForecast1:
    simulation = st.number_input("Enter the number of simulations.", min_value=250.00)
